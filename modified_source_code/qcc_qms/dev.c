...
//ztg paper
	...
	if (skb->sk && tcb && tp && th && likely(!(tcb->tcp_flags & TCPHDR_SYN))) {
		if (strcmp(skb->dev->name, "wifi_eth") == 0) {tp->qccqms.nif = 1;}		// in VB-Android-x86, VB 中 wifi_eth 实际是有线
		else if (strcmp(skb->dev->name, "eth1") == 0) {tp->qccqms.nif = 2;}		// in VB-Android-x86, VB 中 eth1 实际是无线
//		if (strcmp(skb->dev->name, "wlan0") == 0) {tp->qccqms.nif = 1;}			// in Mi8-Android-arm
//		else if (strcmp(skb->dev->name, "rmnet_data1") == 0) {tp->qccqms.nif = 2;}	// in Mi8-Android-arm

		if(tp->qccqms.qoe==1 && tp->qccqms.nif == 2) {									// only use WIFI
			if(tp->qccqms.is_meta_sk==1) th->window = 69;		// 	12 * 2^7 = 12 * 128 = 1536
			else th->window = 0;					// 	 2 * 2^7 =  2 * 128 = 256
			// Do not set th->window of meta_sk to 0, otherwise, the connection will be broken
		}
		else if(tp->qccqms.qoe==2 && tp->qccqms.nif == 1) {									// only use 4G
			if(tp->qccqms.is_meta_sk==1) th->window = 69;		// 	12 * 2^7 = 12 * 128 = 1536
			else th->window = 0;					// 	 2 * 2^7 =  2 * 128 = 256
		}
		else if(tp->qccqms.trigger && tp->qccqms.qoe==3) ;									// use all subflows normally
		else if(tp->qccqms.trigger && (tp->qccqms.qoe==27 || tp->qccqms.qoe==31) && tp->qccqms.ann_win==1) {
			if(tp->qccqms.is_meta_sk==1) th->window = 69;		// 	12 * 2^7 = 12 * 128 = 1536
			else th->window = 0;					// 	 2 * 2^7 =  2 * 128 = 256
		}

		if (tp->qccqms.cap_inter_o-- == 4 && tp->qccqms.qoe!=0 && tp->qccqms.qoe!=255)			// 为了得到论文结果分析所需的完整数据，每发送一个包都输出
			printk(KERN_INFO "QOE_send: name:%s: nif:%d: meta_sk:%d: trigger:%d: qoe:%d: window:%d: wscale:%d: ann_win:%d: sort_rtt:%d: sort_cwnd:%d: rtt:%d: ssthresh:%d: cwnd:%d: delivered:%d: lost:%d: retrans:%d\n", 
				skb->dev->name, tp->qccqms.nif, tp->qccqms.is_meta_sk, tp->qccqms.trigger, tp->qccqms.qoe, th->window, tp->rx_opt.rcv_wscale, tp->qccqms.ann_win, tp->qccqms.sort_rtt, tp->qccqms.sort_cwnd, 
				tp->srtt_us >> 3, tp->snd_ssthresh, tp->snd_cwnd, tp->delivered, tp->lost, tp->total_retrans);
		else if (tp->qccqms.cap_inter_o == 0)
			tp->qccqms.cap_inter_o = 4;
	}

//QOE_send: name:eth1: nif:2: is_meta_sk:0: trigger:0: qoe:1: window:0: rcv_wscale:7: ann_win:0: cnt:0: rtt:0: snd_ssthresh:7: snd_cwnd:1: delivered:0: lost:4: total_retrans:3
//QOE_send: name:eth1: nif:2: is_meta_sk:1: trigger:0: qoe:1: window:12: rcv_wscale:7: ann_win:0: cnt:0: rtt:0: snd_ssthresh:2147483647: snd_cwnd:10: delivered:0: lost:0: total_retrans:0
//QOE_send: name:eth1: nif:2: is_meta_sk:1: trigger:0: qoe:1: window:12: rcv_wscale:7: ann_win:0: cnt:0: rtt:0: snd_ssthresh:7: snd_cwnd:1: delivered:0: lost:4: total_retrans:3

//QOE_send: name:eth1: nif:2: is_meta_sk:1: trigger:0: qoe:3: window:69: rcv_wscale:7: ann_win:0: cnt:0: rtt:1798: snd_ssthresh:2147483647: snd_cwnd:10: delivered:1: lost:0: total_retrans:0
	//----------------------------------------	QCC & QMS
