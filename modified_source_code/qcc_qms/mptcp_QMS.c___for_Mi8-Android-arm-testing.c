 ...
 *
 *	QMS is based on the defualt scheduler of MPTCP.
 *
...
static int mptcp_qms_dont_reinject_skb(const struct tcp_sock *tp, const struct sk_buff *skb)
{
			    ...
			    bool zero_wnd_test, bool *force)
{
	...
	mptcp_for_each_sk(mpcb, sub_sk) {
		...
		if(tp->qccqms.trigger) {
			if(tp->qccqms.qoe==1 && tp->qccqms.nif == 2) continue;						// only use WIFI
			else if(tp->qccqms.qoe==2 && tp->qccqms.nif == 1) continue;					// only use 4G
//			else if(tp->qccqms.qoe==3) ;										// use all subflows normally
			else if((tp->qccqms.qoe==27 || tp->qccqms.qoe==31) && 
					tp->qccqms.ann_win==1 && tp->qccqms.is_meta_sk!=1) continue;			// small file (27), large file (31)
						 					// 条件要有 tp->qccqms.is_meta_sk!=1，保证至少 meta_sk 上可以传递数据，否则 很快中止
						 					// ann_win;	// Artifically deflate announced window to 0, which is used in
		...
		if (!mptcp_qms_dont_reinject_skb(tp, skb))
			unused = true;
		...
		if (mptcp_is_def_unavailable(sk))				//use mptcp_sched.c
			continue;
		...
		// QMS ADD
		//--------------------------------------------------------------------------
		if(tp->qccqms.trigger && (tp->qccqms.sort_rtt == 1 || tp->qccqms.sort_cwnd == 1)) {		// use all subflows normally
 ...
 */
static struct sock *qms_get_available_subflow(struct sock *meta_sk,
					     struct sk_buff *skb,
					     bool zero_wnd_test)
{
...
//			sk = mptcp_to_sock(mptcp);
		mptcp_for_each_sk(mpcb, sub_sk) {
			...
			if (tcp_sk(sk)->mptcp->path_index == mpcb->dfin_path_index &&
			    mptcp_is_available(sk, skb, zero_wnd_test))				//use mptcp_sched.c
				return sk;
		}
	}
...
static void qmssched_init(struct sock *sk)
{
	...
	qms_p->last_rbuf_opti = tcp_time_stamp;
}

static struct mptcp_sched_ops mptcp_sched_qms = {
	.get_subflow = qms_get_available_subflow,
	.next_segment = mptcp_next_segment,			// use mptcp_sched.c
	.init = qmssched_init,
	.name = "qms",
	.owner = THIS_MODULE,
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("QMS (QoE-Driven MPTCP Scheduler) MPTCP");
MODULE_VERSION("0.1");
